...
const addImageToS3 = async(category,categoryImg) =>{
    ...
    const putImageCommand = new PutObjectCommand(
        {
            Bucket: bucketName,
            Key: `categories/${category}.jpg`,
            Body: categoryImg
        }
    )
    await client.send(putImageCommand)
    return `https://${bucketName}.s3.ap-south-1.amazonaws.com/categories/${category}.jpg`
}
...
categoryRoute.patch('/category/img/:categoryName?',uploadCategoryImg.single("categoryImg"),async(req,resp)=>{
    try {
        let imgStr = ""
        if (req.file) {
            imgStr = await addImageToS3(req.params.categoryName, req.file.buffer)
        }
